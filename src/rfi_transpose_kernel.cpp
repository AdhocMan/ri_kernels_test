...
ffi::Error calc_rfi_transpose_cpu_impl(
  ...
  const int64_t n_rfi = rfi_amp_fine_tensor.shape[0];
  const int64_t n_rfi_per_thread = (n_rfi + n_threads - 1) / n_threads;

  std::latch done(n_threads);

  for (int64_t thread_id = 0; thread_id < n_threads; ++thread_id) {
    const int64_t i_rfi_start = thread_id * n_rfi_per_thread;
    if (i_rfi_start >= n_rfi) {
      done.count_down();
      continue;
    }
    thread_pool.Schedule([&, i_rfi_start]() {
      const int64_t n_rfi_this_thread =
          std::min(i_rfi_start + n_rfi_per_thread, n_rfi) - i_rfi_start;

      Tensor3D<const std::complex<double> *> rfi_amp_fine_tensor_th(
          &rfi_amp_fine_tensor(i_rfi_start, 0, 0), n_rfi_this_thread,
          rfi_amp_fine_tensor.shape[1], rfi_amp_fine_tensor.shape[2]);
      Tensor3D<const double *> rfi_phase_tensor_th(
          &rfi_phase_tensor(i_rfi_start, 0, 0), n_rfi_this_thread,
          rfi_phase_tensor.shape[1], rfi_phase_tensor.shape[2]);
      Tensor3D<std::complex<double> *> rfi_amp_fine_grad_tensor_th(
          &rfi_amp_fine_grad_tensor(i_rfi_start, 0, 0), n_rfi_this_thread,
          rfi_amp_fine_grad_tensor.shape[1], rfi_amp_fine_grad_tensor.shape[2]);
      Tensor3D<double *> rfi_phase_grad_tensor_th(
          &rfi_phase_grad_tensor(i_rfi_start, 0, 0), n_rfi_this_thread,
          rfi_phase_grad_tensor.shape[1], rfi_phase_grad_tensor.shape[2]);

      RI_KERNELS_EXPORT_AND_DISPATCH_T(rfi_transpose_kernel_opt)
      (n_int_f, n_int_t, a1_tensor, a2_tensor, rfi_amp_fine_tensor_th,
       rfi_phase_tensor_th, rfi_grad_tensor, rfi_amp_fine_grad_tensor_th,
       rfi_phase_grad_tensor_th);
